...
世界工坊可先选择「自定生存与目标」或「阶位成长」模板。前者会生成 1~3 项世界资源、逐年变化、数值目标和失败阈值，并可选择人物是否自然衰老；程序在推进时逐年结算，并在达到条件时自动结束。生成前会检查资源引用与初始值，预览中会展示规则。既有自建世界和存档仍沿用保存时的规则；新规则只用于新生成的世界。它仍只支持当前人物的一局，跨代接班和任意脚本规则尚未提供。改造记录见[产品结构与规则重组方案](docs/产品结构与规则重组方案.md)。
